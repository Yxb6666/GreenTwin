...
import {
  focusMapOnTownship,
  isTownshipInteractionBlocked,
  TOWNSHIP_FOCUS_START_EVENT,
} from "./mapFocus";
...
export function useLeafletMap(container: Ref<HTMLElement | null>) {
  ...
  function getTownshipVisualState(entry: TownshipLayerEntry) {
    return resolveTownshipVisualState(
      entry.name,
      selectedTownship.value,
      hoveredTownship.value,
    );
  }

  function refreshTownshipLabel(
    entry: TownshipLayerEntry,
  ...
  function refreshTownshipStyles() {
    townshipLayers.forEach((entry) => {
      ...
      entry.parts.forEach((part) =>
        part.setStyle(getTownshipPathStyle(state, entry.baseStyle)),
      );
  ...
  function addCountyFocusContext(
    instance: L.Map,
    features: Awaited<ReturnType<typeof loadTownshipFeatures>>,
  ) {
    const boundaryRings = filterCountyBoundaryArtifacts(
      buildCountyBoundaryRings(features),
    );
  ...
  function setTownshipLabelPlacement(
    name: string,
    options: Pick<L.TooltipOptions, "direction" | "offset" | "className">,
  ...
  function resetTownshipLabelPlacements() {
    townshipLayers.forEach((entry) => {
      ...
      entry.labelPart
        .unbindTooltip()
        .bindTooltip(content, entry.defaultTooltipOptions);
  ...
  function renderTownshipFeatures(
    ...
    townshipLayers
      .splice(0)
      .forEach((entry) => entry.parts.forEach((part) => part.remove()));
    ...
    const labelCandidates = new Map<
      string,
      { polygon: L.Polygon; area: number }
    >();

    orderTownshipRingParts(displayFeatures).forEach(
      ({ feature, ring, area }) => {
        const label = getTownshipLabel(feature);
        const townshipIsInteractive = Boolean(
          interactionOptions.townshipFocus && label,
        );
        const townshipBaseStyle = townshipIsInteractive
          ? TOWNSHIP_NORMAL_STYLE
          : TOWNSHIP_LEGACY_STYLE;
        ...
        if (townshipIsInteractive && label) {
          ...
          polygon.on("click", (event) => {
            if (selectTownship(instance, label))
  ...
  function activateBaseLayer(layer: L.TileLayer, mode: BaseMapMode) {
    ...
    if (activeBaseLayer && activeBaseLayer !== layer)
      map.value.removeLayer(activeBaseLayer);
  ...
  function setBaseMap(mode: BaseMapMode) {
    ...
    if (option.source === "supermap") {
      activeBaseMap.value = mode;
      return superMapBaseLayer
        ? activateBaseLayer(superMapBaseLayer, mode)
        : true;
    }
    ...
    if (!layer) {
      const tileUrl =
        option.tileUrl ??
        (option.arcgisStyle
          ? buildArcGisTileUrl(option.arcgisStyle, arcgisAccessToken)
          : "");
      ...
      layer.on("tileerror", () => {
        if (!disposed)
          error.value = `${option.name}加载失败，请检查 accessToken、域名限制与网络连接。`;
      });
  ...
  function setLandUseRaster(active: boolean, overlay: LandUseRasterOverlay) {
    ...
    landUseRasterLayer = L.tileLayer(
      `${collectionUrl}/tile.png?transparent=true&cacheEnabled=true&z={z}&x={x}&y={y}`,
      {
    ...
    landUseRasterLayer.on("tileerror", () => {
      if (!disposed)
        error.value =
          "土地利用栅格加载失败，请检查 LankaoLand 影像服务与跨域配置。";
    });
  ...
  async function initialize(
    ...
    try {
      ...
      if (interactionOptions.townshipFocus)
        instance.getContainer().classList.add("map--township-focus");
      ...
      townshipPane.style.pointerEvents = interactionOptions.townshipFocus
        ? "auto"
        : "none";
      ...
      void loadSuperMapLeaflet(sdkUrl)
        .then((superMapLeaflet) => {
          if (disposed) return;
          superMapBaseLayer = superMapLeaflet.supermap!.tiledMapLayer(
            serviceUrl,
            {
              transparent: false,
              crossOrigin: true,
                  ...
                  void loadCountyFeatures(overlayServiceUrl)
                    .then((countyFeatures) => {
                      if (!disposed && !countyFocusContextAdded) {
                        countyFocusContextAdded = addCountyFocusContext(
                          instance,
                          countyFeatures,
                        );
                      }
