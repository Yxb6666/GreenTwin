...
const assistantContext = computed<DecisionAssistantContext>(() => ({
  module: '三生评估',
  scopeLabel: `${selectedTown.value.name} · ${dimensionMeta[activeDimension.value].label}`,
  updatedAt: new Date().toISOString(),
  data: {
    selectedTown: selectedTown.value.name,
    selectedDimension: dimensionMeta[activeDimension.value].label,
    scores: selectedTown.value.scores,
    countyComposite: countyScores.value.composite,
        ...
        <section class="report-dialog" role="dialog" aria-modal="true" :aria-label="report.title">
          ...
          <div class="report-dialog__meta">
            ...
            <span
              >生成时间
              {{
                new Date(reportMeta.generatedAt).toLocaleString('zh-CN', {
                  hour12: false,
                })
              }}</span
            >
          ...
          <div class="report-dialog__body">
            ...
            <section>
              ...
              <div class="dimension-report-grid">
                <article v-for="item in report.dimensionAnalysis" :key="item.dimension">
                  ...
                  <p>{{ item.assessment }}</p>
                  <ul>
                    <li v-for="evidence in item.evidence" :key="evidence">
                      {{ evidence }}
                    </li>
                  </ul>
                </article>
            ...
            <div class="report-two-column">
              ...
              <section>
                <h3>关键短板</h3>
                <ol>
                  <li v-for="item in report.weaknesses" :key="item">
                    {{ item }}
                  </li>
                </ol>
              </section>
...
.score-orbit {
  ...
  border-radius: 50%;
  box-shadow:
    inset 0 0 24px rgba(61, 214, 196, 0.13),
    0 0 18px rgba(61, 214, 196, 0.1);
  text-align: center;
}

.score-orbit strong {
  color: var(--cyan);
  font: 27px var(--font-data);
}
.score-orbit span {
  color: var(--text-soft);
  font-size: 9px;
...
.county-strip span:nth-of-type(2) {
  margin-left: 16px;
}
.county-strip b {
  color: var(--text);
...
.report-dialog__body::-webkit-scrollbar {
  width: 7px;
}
.report-dialog__body::-webkit-scrollbar-track {
  background: rgba(255, 255, 255, 0.03);
}
.report-dialog__body::-webkit-scrollbar-thumb {
  border-radius: 99px;
  background: rgba(61, 214, 196, 0.42);
}
...
.report-dialog__body ol {
  margin: 8px 0 0;
  padding-left: 20px;
}
.report-lead {
  border-left: 3px solid var(--cyan) !important;
  background: rgba(61, 214, 196, 0.055) !important;
}
...
.dimension-report-grid article {
  padding: 12px;
  border: 1px solid var(--line);
}
.dimension-report-grid header {
  display: flex;
  align-items: center;
}
.dimension-report-grid header strong {
  font-size: 12px;
}
.dimension-report-grid header b {
  margin-left: auto;
  color: var(--cyan);
...
.recommendation-list header strong {
  color: var(--text);
  font-size: 11px;
}
.recommendation-list header span {
  margin-left: auto;
  font: 9px var(--font-data);
}
...
.priority-低 {
  color: var(--green);
  background: rgba(120, 215, 135, 0.12);
}
