...
import { createGovernanceHeatLayer } from './heatmap'
...
function refreshIssueMarkers() {
  ...
  const visibleIds = heatmapActive.value
    ? new Set<string>()
    : new Set(filtered.value.map((issue) => issue.id))
...
function refreshHeatmap() {
  ...
  if (!heatmapActive.value || !filtered.value.length) return
  heatLayer = createGovernanceHeatLayer(filtered.value).addTo(mapInstance)
}

function toggleHeatmap() {
  if (!heatmapActive.value && !filtered.value.length) {
    notify('当前筛选条件下没有可分析的问题')
    return
  }
  ...
  notify(
    heatmapActive.value
      ? `热力图分析已开启，共分析 ${filtered.value.length} 个问题点`
      : '热力图分析已退出',
  )
}
...
async function exportIssues() {
  ...
  try {
    ...
    anchor.href = url
    anchor.download = '乡村治理问题要素_SHP.zip'
    anchor.click()
    URL.revokeObjectURL(url)
    notify(`已导出 ${filtered.value.length} 个治理问题要素（SHP）`)
  } catch (cause) {
    notify(
      cause instanceof Error
        ? `SHP 导出失败：${cause.message}`
        : 'SHP 导出失败',
    )
  } finally {
...
watch(
  [map, issues, filtered, selectedId, aiHighlightedIds, heatmapActive],
  refreshIssueMarkers,
  {
    immediate: true,
  },
...
watch([map, filtered], () => {
  if (heatmapActive.value) refreshHeatmap()
})
...
onBeforeUnmount(() => {
  ...
  stopSpatialDrawing()
  if (heatLayer && map.value) map.value.removeLayer(heatLayer)
  issueMarkers.clear()
})
</script>
  ...
  <main class="screen-page governance-page">
    ...
    <div class="governance-layout">
      <aside class="governance-left">
        ...
        <PanelCard title="筛选条件" :meta="spatialQueryLabel">
          ...
          <button
            ...
            @click="exportIssues"
          >
            {{ isExporting ? 'SHP 生成中…' : '导出当前数据（SHP）' }}
          </button>
        </PanelCard>
      </aside>
      ...
      <aside class="governance-right">
        ...
        <PanelCard title="热力图分析" :meta="`${filtered.length} 个问题点`">
          <div class="heatmap-analysis">
            <p>
              按问题密度与紧急程度识别治理热点，结果随当前筛选条件实时联动。
            </p>
            <button
              ...
              @click="toggleHeatmap"
            >
              <span class="heatmap-button__icon" aria-hidden="true"
                ><i /><i /><i
              /></span>
              {{ heatmapActive ? '退出热力图分析' : '开始热力图分析' }}
            </button>
            <div class="heatmap-legend" :class="{ active: heatmapActive }">
              <span>低密度</span><i aria-hidden="true" /><span>高密度</span>
            </div>
          </div>
        </PanelCard>
      </aside>
...
.issue-detail__entry {
  ...
  background: linear-gradient(
    90deg,
    rgba(61, 214, 196, 0.14),
    rgba(61, 214, 196, 0.05)
  );
  font-size: 9px;
  white-space: nowrap;
  cursor: pointer;
...
.export-button {
  ...
  margin-top: 10px;
  color: #d7fff9;
  border: 1px solid rgba(61, 214, 196, 0.48);
  border-radius: 4px;
...
.export-button:hover:not(:disabled) {
  ...
  background: rgba(61, 214, 196, 0.18);
}
...
:global(.governance-heatmap-canvas) {
...
:global(.governance-issue-pin.is-active) {
  z-index: 1;
  filter: drop-shadow(0 4px 5px rgba(0, 0, 0, 0.68))
...
.heatmap-button {
  ...
  background: linear-gradient(
    ...
    rgba(34, 111, 103, 0.08)
  );
  box-shadow: inset 0 0 12px rgba(61, 214, 196, 0.04);
  font-size: 10px;
  cursor: pointer;
...
.heatmap-button.active {
  ...
  box-shadow: 0 0 14px rgba(61, 214, 196, 0.12);
}

.heatmap-button__icon {
  position: relative;
  display: inline-flex;
  align-items: end;
  width: 17px;
...
.heatmap-button__icon i:nth-child(1) {
  height: 7px;
}
.heatmap-button__icon i:nth-child(2) {
  height: 14px;
}
.heatmap-button__icon i:nth-child(3) {
  height: 10px;
}
...
.heatmap-legend i {
  ...
  box-shadow: 0 0 7px rgba(255, 150, 63, 0.18);
}
.empty-state {
  color: var(--text-soft);
  font-size: 10px;
