...
function redrawCurrentPath(cursor?: L.LatLng) {
  ...
  previewPath =
    activeMeasurement.value === "distance"
      ? L.polyline(previewPoints, { ...pathOptions, opacity: 0.55 }).addTo(
          layer,
        )
      : L.polygon(previewPoints, { ...pathOptions, opacity: 0.55 }).addTo(
          layer,
        );
}
...
function finishMeasurement(event?: L.LeafletMouseEvent) {
  ...
  if (type === "distance" && points.length >= 2) {
    const meters = points
      ...
      .reduce(
        (total, point, index) =>
          total + props.map!.distance(points[index]!, point),
        0,
      );
...
async function exportMap() {
  ...
  try {
    const tiles = [
      ...container.querySelectorAll<HTMLImageElement | HTMLCanvasElement>(
        ".leaflet-map-pane .leaflet-tile, .leaflet-overlay-pane canvas",
      ),
    ];
    for (const tile of tiles)
      await drawDomImage(context, tile, container, root);
    ...
    context.fillText(
      props.exportName ?? "GreenTwin 地图视图",
      16,
      root.height - 20,
    );
  ...
  <div class="map-toolbox" aria-label="地图常用工具">
    <div class="map-toolbox__rail" role="toolbar" aria-label="地图操作">
      ...
      >
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path
            d="M20 7v5h-5M4 17v-5h5m9.2-3A7 7 0 0 0 6.6 7L4 12m16 0-2.6 5a7 7 0 0 1-11.6-1"
          />
        </svg>
        <span>刷新地图</span>
      </button>
    ...
    <Transition name="tool-panel">
      <div
        v-if="baseMapMenuOpen"
        id="basemap-menu"
        class="map-toolbox__panel basemap-panel"
      >
          ...
          <button
            ...
            :class="[
              `preview--${option.key}`,
              {
                ...
                unavailable:
                  requiresArcGisAccessToken(option) && !arcgisAvailable,
              },
            ]"
            :aria-disabled="
              requiresArcGisAccessToken(option) && !arcgisAvailable
...
.basemap-grid button > i {
  ...
  background-image:
    linear-gradient(
      25deg,
      transparent 45%,
      rgba(209, 229, 166, 0.8) 46% 54%,
      transparent 55%
    ),
    linear-gradient(
      118deg,
      transparent 52%,
      rgba(100, 165, 183, 0.82) 53% 61%,
      transparent 62%
    ),
    radial-gradient(circle at 28% 38%, #9fba75 0 14%, transparent 15%);
}
...
.preview--night > i {
  filter: grayscale(0.8) invert(0.82) hue-rotate(145deg) brightness(0.42)
    contrast(1.7);
}
...
:global(.map-base--night) {
  filter: grayscale(0.78) invert(0.82) hue-rotate(145deg) brightness(0.38)
    contrast(1.75);
}
