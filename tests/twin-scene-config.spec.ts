...
describe('三生模拟场景配置', () => {
  ...
  it('服务圈分析使用精简的单路径操作卡并即时标记选点', async () => {
    ...
    expect(source).toContain('class="analysis-guide-card"')
    expect(source).toContain('serviceAnalysisSteps')
    expect(source).toContain('分析指引')
    expect(source).toContain('grid-template-rows: max-content max-content minmax(190px, 1fr)')
    ...
    expect(source).toContain('point.height + parkModelHeightOffset')
    expect(source).toContain('isochroneSurfaceOffset')
    expect(source).toContain('featureIndex * isochroneLayerGap')
    expect(source).not.toContain('height: 8 + featureIndex * 2')
