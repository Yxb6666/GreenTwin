...
describe("主控专题地图指标", () => {
  ...
  it("为人口密度专题生成分级设色指标", () => {
    ...
    expect(
      masterMapThemeLegends.population.map((item) => item.color),
    ).toContain(metric.color);
  });
  ...
  it("为 POI 专题生成聚合点详情", () => {
    ...
    expect(metric.breakdown?.reduce((sum, item) => sum + item.value, 0)).toBe(
      metric.value,
    );
    expect(metric.radius).toBeCloseTo(
      Math.min(21, Math.max(11, 8 + Math.sqrt(metric.value) * 1.1)),
    );
  });
  ...
  it("人口与三生主题按当前行政区实际值动态分位设色", () => {
    ...
    sansheng.forEach((metric, index) => {
      expect(metric.value).toBe(
        scoreTown(towns[index]!, DEFAULT_DIMENSION_WEIGHTS).composite,
      );
    });
  ...
  it("治理问题为零时不生成聚合半径", () => {
    expect(
      resolveTownshipThemeMetric("governance", yifengFeature, 0).radius,
    ).toBeUndefined();
  });
});
